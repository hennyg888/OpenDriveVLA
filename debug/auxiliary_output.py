...
BEVFUSION_CONFIG = "projects/configs/bevfusion_track_map/bevfusion.py"
BEVFUSION_CKPT = "/home/s56cai/ckpt/bevfusion/bevfusion-det.pth"
TRACK_MAP_FORMER_CONFIG = "projects/configs/bevfusion_track_map/track_map_former.py"
TRACK_MAP_FORMER_CKPT = "/home/s56cai/ckpt/uniad_stage1/uniad_base_track_map.pth"
...
JSON_PREFIX = os.path.join(
    "test",
    os.path.basename(TRACK_MAP_FORMER_CONFIG).replace(".py", ""),
    "latest",
)

# ===== Chunk writing config =====
CHUNK_SIZE = 50  # 每个 chunk 存多少条结果（越小越安全，文件越多）
# ===============================
...
def pack_for_nuscenes_e2e(out: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 如果你不需要 planning 相关轨迹，这俩会非常大；可按需注释
    if "traj" in rt:
        packed["traj"] = rt["traj"]
...
def main():
    ...
    cfg_bf = Config.fromfile(BEVFUSION_CONFIG)
    ...
    from projects.mmdet3d_plugin.models.builder import build_model as build_plugin_model
    from mmdet3d.models import build_model as build_mmdet3d_model

    print(f"[rank{rank}] build bevfusion...", flush=True)
    bevfusion = build_plugin_model(cfg_bf.model).to(device).eval()
    ckpt_bf = torch.load(BEVFUSION_CKPT, map_location="cpu")
    bevfusion.load_state_dict(ckpt_bf.get("state_dict", ckpt_bf), strict=False)
    print(f"[rank{rank}] bevfusion ready", flush=True)

    print(f"[rank{rank}] build track_map_former...", flush=True)
    cfg_tm = Config.fromfile(TRACK_MAP_FORMER_CONFIG)
    cfg_tm.model.pop("train_cfg", None)
    cfg_tm.model.pop("test_cfg", None)
    model_tm = build_mmdet3d_model(cfg_tm.model).to(device).eval()
    ckpt_tm = torch.load(TRACK_MAP_FORMER_CKPT, map_location="cpu")
    model_tm.load_state_dict(ckpt_tm.get("state_dict", ckpt_tm), strict=False)
    print(f"[rank{rank}] track_map_former ready", flush=True)
