...
# NuScenes full category name → detection class index (for per-class GT coloring)
_NUSC_CATEGORY_TO_CLASS_IDX = {
    'vehicle.car':                  0,
    'vehicle.truck':                1,
...
def nusc_box_name_to_color(name: str) -> np.ndarray:
    """Return an RGB color (0-1 range) for a NuScenes annotation category name."""
    for prefix, idx in _NUSC_CATEGORY_TO_CLASS_IDX.items():
        if name.startswith(prefix):
            return color_mapping[idx % len(color_mapping)]
    # unknown category: hash-based fallback
    return color_mapping[hash(name) % len(color_mapping)]
