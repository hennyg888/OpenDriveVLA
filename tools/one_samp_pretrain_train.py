...
def main():
    ...
    print(f"Optimizer initialized: {optimizer.__class__.__name__} | lr: {optimizer.param_groups[0]['lr']} | param_groups: {len(optimizer.param_groups)}")

    # Build a runner so we can see what training hooks would be registered
    ...
    for h in runner._hooks:
        ...
        if 'GradientCumulativeOptimizerHook' in h.__class__.__name__:
            print(f"    grad accumulation steps: {h.cumulative_iters}")
    ...
    try:
        ...
        # Print summary and prepare loss for backward
        loss_tensors = [v for k, v in losses.items() if 'loss' in k.lower() and torch.is_tensor(v)]
        if not loss_tensors:
            raise RuntimeError("No tensor losses found for backward pass")
        ...
        total_loss.backward()
        optimizer.step()
        print("Performed one backward + optimizer step")
