...
def process_sample_token(sample_token: str):
    global nusc, nusc_maps
    ...
    if map_elem_count == 0:
        return None, None
    return out_item, layer_count_item
...
def main():
    ...
    sorted_tok_list = list(stage1_tokens)
    sorted_tok_list.sort()

    with (
        out_p.open("w", encoding="utf-8") as f,
        layer_p.open("w", encoding="utf-8") as f_layer,
    ):
        with ProcessPoolExecutor(initializer=init_worker, max_workers=WORKER_COUNT) as executor:
            for out_item, layer_count_item in executor.map(process_sample_token, sorted_tok_list):
                if out_item is not None:
                    f.write(json.dumps(out_item, ensure_ascii=False) + "\n")
                if layer_count_item is not None:
