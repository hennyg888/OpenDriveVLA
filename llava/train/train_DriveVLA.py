...
from llava.train.train import smart_tokenizer_and_embedding_resize, safe_save_model_for_hf_trainer, get_peft_state_maybe_zero_3, get_peft_state_non_lora_maybe_zero_3, find_all_linear_names, ModelArguments, DataArguments, TrainingArguments, get_model
...
def train_DriveVLA(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if model_args.tune_mm_mlp_adapter:
                for p in model.get_model().mm_projector_track.parameters():
                    p.requires_grad = True
                for p in model.get_model().mm_projector_scene.parameters():
                    p.requires_grad = True
                for p in model.get_model().mm_projector_map.parameters():
                    p.requires_grad = True
            ...
            model.requires_grad_(False)
            vision_tower.requires_grad_(False)
            model.get_model().mm_projector_track.requires_grad_(False)
            model.get_model().mm_projector_scene.requires_grad_(False)
            model.get_model().mm_projector_map.requires_grad_(False)
        ...
        if training_args.bits in [4, 8]:
            model.get_model().mm_projector_track.to(dtype=compute_dtype, device=training_args.device)
            model.get_model().mm_projector_scene.to(dtype=compute_dtype, device=training_args.device)
            model.get_model().mm_projector_map.to(dtype=compute_dtype, device=training_args.device)

        model.config.mm_use_im_start_end = data_args.mm_use_im_start_end = model_args.mm_use_im_start_end
        model.config.mm_projector_lr = training_args.mm_projector_lr
        model.config.mm_projector_track_lr = training_args.mm_projector_lr
        model.config.mm_projector_scene_lr = training_args.mm_projector_lr
        model.config.mm_projector_map_lr = training_args.mm_projector_lr
    ...
    train_dataset = LLaVANuScenesDataset(tokenizer, data_args, uniad_cfg.data.train_llava_and_vision_tower, llava_train_mode=True, use_uniad_pth=data_args.use_uniad_pth, in_nuscenes_order=data_args.in_nuscenes_order)
