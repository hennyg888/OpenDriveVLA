...
def inference_planning_oriented_vlm(args):
    ...
    # processed_ids = read_processed_ids(args.resume_from_output) if args.resume_from_output else set()
    # print(f"Loaded {len(data_all)} data items, {len(processed_ids)} already processed")

    # unprocessed_data = [data_item for data_item in data_all if data_item['id'] not in processed_ids]
...
def main():
    ...
    data_path_default = "data/drivevla/traj_conv_val.json"
