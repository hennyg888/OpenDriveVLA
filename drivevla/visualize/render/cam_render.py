    ...
    def project_to_cam(self,
        ...
        for agent in agent_prediction_list:
            ...
            if project_traj:
                ...
                else:  # box.is_sdc == True
                    box.pred_trajs = []
                    for i in range(agent.pred_traj.shape[0]):
                        pred_traj = np.zeros((agent.pred_traj[i].shape[0]+1, 3))
                        pred_traj[:, 0] = agent.pred_center[0]
                        pred_traj[:, 1] = agent.pred_center[1]
                        pred_traj[:, 2] = agent.pred_center[2] - \
                            agent.pred_dim[2]/2
                        pred_traj[1:, :2] += agent.pred_traj[i][:, :2]
                        pred_traj = (Quaternion(
                            lidar_cs_record['rotation']).rotation_matrix @ pred_traj.T).T
                        pred_traj += np.array(
                            lidar_cs_record['translation'])[None, :]
                        box.pred_trajs.append(pred_traj)
        ...
        for i, box in enumerate(boxes):
            ...
            if project_traj:
                if not box.is_sdc:
                    ...
                    for i in range(len(box.pred_trajs)):
                        box.pred_trajs[i] += -np.array(cs_record['translation'])[None, :]
                        box.pred_trajs[i] = (Quaternion(
                            cs_record['rotation']).inverse.rotation_matrix @ box.pred_trajs[i].T).T
        ...
        for i, cam in enumerate(self.cams):
            ...
            for j, box in enumerate(box_list):
                if not box.is_sdc:
                    ...
                    for traj_idx, pred_traj in enumerate(box.pred_trajs):
                        traj_points = pred_traj[:, :3]
                        ...
                        traj_points = traj_points[:2, in_range_mask]
                        if render_sdc:
                            c = color_mapping[traj_idx % len(color_mapping) + 1]
                            self.axes[i//3, i % 3].scatter(
                                traj_points[0], traj_points[1], color=c, s=75)
                        else:
                            continue
            self.axes[i//3, i % 3].set_xlim(0, imsize[0])
            self.axes[i//3, i % 3].set_ylim(imsize[1], 0)
