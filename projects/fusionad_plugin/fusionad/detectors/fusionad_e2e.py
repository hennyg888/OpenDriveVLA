...
class FusionAD(FusionADTrack):
    ...
    def get_results_for_vlm(self, data):
        ...
        Mirrors track_map_former.get_results_for_vlm (UniAD) but for FusionAD.
        Populates track_gt_inds_to_embed_idx by IoU-matching detected boxes to
        GT boxes so the VLM can look up a specific object's track embedding.

        Returns:
            dict with keys:
                result_track:
                    track_query_embeddings  Tensor[N, 256]
                    img_feat_2D             Tensor[1, N_cam, C, H, W]
                    track_gt_inds_to_embed_idx  Dict[int, int]
                result_seg:
                    chosen_output_query_things  Tensor[N_things, D]
                    output_query_stuff          Tensor[1, D]
                sample_token: str
                scene_token:  str
        """
        img       = data['img']
        ...
        img_feat_2D = feat.unsqueeze(0).view(
            1, feat.shape[0], feat.shape[1], feat.shape[2], feat.shape[3]
        )

        # Build track_gt_inds_to_embed_idx via IoU matching.
        # gt_bboxes_3d / gt_inds structure after collate+remove_datacontainer:
        #   [[LiDARInstance3DBoxes]]  (batch=0, aug=0)
        #   [[Tensor([N])]]
        track_gt_inds_to_embed_idx: Dict[int, int] = {}
        gt_bboxes_3d = data.get('gt_bboxes_3d')
        gt_inds       = data.get('gt_inds')
