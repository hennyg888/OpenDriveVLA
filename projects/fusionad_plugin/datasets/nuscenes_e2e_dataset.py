...
class NuScenesE2EDataset(NuScenesDataset):
    ...
    def _fix_path(self, path):
        ...
        # 旧工程绝对路径前缀
        old_abs_prefixes = [
            '/mnt/petrelfs/yangjiazhi/e2e_proj/data/nuscenes/',
            '/mnt/petrelfs/yangjiazhi/e2e_proj/data/nuscenes',
        ]

        # 1) 先处理旧绝对路径
        for old_prefix in old_abs_prefixes:
            if path.startswith(old_prefix):
                rel = path[len(old_prefix):].lstrip('/')
                return os.path.join(self.data_root, rel)

        # 2) 处理旧相对路径前缀
        for bad_prefix in ['./UniAD/', 'UniAD/']:
            if path.startswith(bad_prefix):
                path = path[len(bad_prefix):]
    ...
    def load_annotations(self, ann_file):
        ...
        # 🔥 关键修复：判断类型
        if isinstance(ann_file, (str, bytes, os.PathLike)):
            # 传进来的是路径（正常情况）
            data = pickle.loads(self.file_client.get(ann_file))
        else:
            # 传进来的是 file object（你现在的情况）
    ...
    def prepare_test_data(self, index):
        ...
        input_dict['pts_filename'] = self._fix_path(input_dict['pts_filename'])
        input_dict['img_filename'] = [self._fix_path(p) for p in input_dict['img_filename']]

        for sweep in input_dict['sweeps']:
            sweep['data_path'] = self._fix_path(sweep['data_path'])
    ...
    def get_data_info(self, index):
        ...
        input_dict = dict(
            ...
            pts_filename=self._fix_path(info['lidar_path']),
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                image_paths.append(self._fix_path(cam_info['data_path']))
