...
def accumulate_Hungarian(gt_boxes: EvalBoxes,
    ...
    """
    Average Precision over predefined different recall thresholds for a single distance threshold.
    Uses Hungarian (optimal) matching instead of greedy matching.
    """
    npos = len([1 for gt_box in gt_boxes.all if gt_box.detection_name == class_name])
    if verbose:
        print("Found {} GT of class {} out of {} total across {} samples.".
    ...
    for sample_token in tqdm.tqdm(gt_boxes.sample_tokens):
        ...
        if len(gt_agents) > 0:
            matched_of_gt_box = np.ones(len(gt_agents), dtype=int) * -1
    ...
    rec_interp = np.linspace(0, 1, DetectionMotionMetricData.nelem)
    prec = np.interp(rec_interp, rec, prec, right=0)
    conf = np.interp(rec_interp, rec, conf, right=0)
    rec = rec_interp
