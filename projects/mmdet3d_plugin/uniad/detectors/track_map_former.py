...
class Track_Map_Former(UniADTrack):
    ...
    def _forward_single_frame_train(
        ...
        for i in range(nb_dec):
            ...
            all_query_embeddings.append(query_feats[i][0].detach())
            all_matched_indices.append(matched_indices)
            all_instances_pred_logits.append(output_classes[i, 0].detach())
            all_instances_pred_boxes.append(output_coords[i, 0].detach())   # Not used
    ...
    def forward_train(
        ...
        **kwargs):
        ...
        bev_embed = outs_track["bev_embed"].detach()  # Detach BEV embedding to save memory for seg head
    ...
    def _forward_single_frame_track_inference(
        ...
        """ update track base """
        self.track_base.update(track_instances, iou_thre=None)
