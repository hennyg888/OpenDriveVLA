...
from projects.mmdet3d_plugin.uniad.modules.decoder import CustomMSDeformableAttention
...
class UniBEVTransformer(BaseModule):
    ...
    @auto_fp16(apply_to=('img_mlvl_feats', 'pts_mlvl_feats', 'bev_queries', 'bev_pos'))
    def encode_bev(self,
                   img_mlvl_feats,
                   pts_mlvl_feats,
                   bev_queries,
                   ...
                   bev_pos=None,
                   **kwargs):
        """Run only the BEV encoder (ImgEncoder + PtsEncoder) and multi-modal fusion.
        Skip the detection decoder entirely. Used by UniADUniBEV to extract BEV
        features for downstream track and segmentation heads.

        Returns:
            fused_bev_embed (Tensor): shape [bs, bev_h*bev_w, embed_dims]
        """
        # Apply modality dropout (identical logic to forward())
        self.l_flag = 1
        self.c_flag = 1
        ...
        # dual_queries not supported in encode_bev (only used with detection decoder)
        bev_queries_img = bev_queries_pts = bev_queries.unsqueeze(1).repeat(1, bs, 1)

        # Camera BEV encoding via ImgEncoder
        if img_mlvl_feats is not None:
            img_feat_flatten, img_spatial_shapes, img_level_start_index = \
                self._pre_process_img_feats(img_mlvl_feats, bev_queries_img)
        ...
        # LiDAR BEV encoding via PtsEncoder
        if pts_mlvl_feats is not None:
            pts_feat_flatten, pts_spatial_shapes, pts_level_start_index = \
                self._pre_process_pts_feats(pts_mlvl_feats, bev_queries_pts)
        ...
        # Feature normalization and multi-modal fusion (identical to forward())
        img_bev_embed, pts_bev_embed, _ = self.channel_feature_norm(img_bev_embed, pts_bev_embed)
        img_bev_embed, pts_bev_embed, _ = self.spatial_feature_norm(img_bev_embed, pts_bev_embed)
        fused_bev_embed = self.multi_modal_fusion(img_bev_embed, pts_bev_embed)

        # Return [bs, H*W, embed_dims] — caller permutes to [H*W, bs, C] as needed
        return fused_bev_embed
