        ...
        if prev_bev is not None:
            ...
            if self.rotate_prev_bev:
                for i in range(bs):
                    ...
                    # grid_sampler_2d_cuda does not support bfloat16.
                    # Disable autocast entirely for the rotate call: within
                    # torch.autocast(bfloat16), affine_grid (called inside
                    # rotate) is on the whitelist and produces a bf16 grid,
                    # which then promotes the image to bf16 in grid_sample.
                    _bev_dtype = tmp_prev_bev.dtype
                    with torch.autocast('cuda', enabled=False):
                        tmp_prev_bev = rotate(tmp_prev_bev.float(),
                                              rotation_angle,
                                              center=self.rotate_center)
                    tmp_prev_bev = tmp_prev_bev.to(_bev_dtype)
