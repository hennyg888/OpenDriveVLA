...
class SegDeformableTransformer(Transformer):
    ...
    def forward(self,
        ...
        # mmcv built-in MultiScaleDeformAttn CUDA kernel doesn't support bf16;
        # SinePositionalEncoding produces float32 causing query dtype promotion.
        # Temporarily cast encoder to float32 (same pattern as pts_backbone).
        _enc_bf16 = next(self.encoder.parameters(), None)
        _enc_bf16 = (_enc_bf16 is not None and _enc_bf16.dtype == torch.bfloat16)
        if feat_flatten.dtype == torch.bfloat16:
