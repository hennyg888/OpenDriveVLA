...
# 训练前保存初始权重为 epoch_0.pth，训练时每 N iter 打印 encoder 一个参数标量（确认 step 是否更新 encoder）
custom_hooks = [
    dict(type='SaveInitialCheckpointHook', priority='VERY_HIGH', filename='epoch_0.pth'),
    dict(type='DebugEncoderParamHook', interval=20),
    # 每个 epoch 存完 ckpt 后：校验 ckpt vs 内存一致，并比较 epoch_{n-1} vs epoch_{n} 的 encoder 差异统计
    dict(type='VerifyEncoderCheckpointHook', priority='BELOW_NORMAL', compare_prev_epoch=True, topk=8),
]
load_from = None
resume_from = None
