...
data_root = "/home/s56cai/OpenDriveVLA/data/nuscenes/"
info_root = "/home/s56cai/OpenDriveVLA/data/infos/"
...
train_fusionad_with_track_gt_pipeline = [
    # LiDAR loading (no augmentation — deterministic for offline feature extraction)
    dict(
        type='LoadPointsFromFile',
        coord_type='LIDAR',
        load_dim=5,
    ...
    dict(
        ...
        remove_close=True),
    dict(type='PointsRangeFilter', point_cloud_range=point_cloud_range),
    # Image loading (no augmentation)
    dict(type='LoadMultiViewImageFromFilesInCeph', to_float32=True,
         file_client_args=file_client_args, img_root=data_root),
    dict(type='NormalizeMultiviewImage', **img_norm_cfg),
    dict(type='PadMultiViewImage', size_divisor=32),
    # Annotation loading (GT track info required for instance-level alignment)
    dict(type='LoadAnnotations3D_E2E',
         with_bbox_3d=True,
         with_label_3d=True,
         with_attr_label=False,
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='CustomCollect3D', keys=[
                ...
                'sdc_planning_mask',
                'command',
                # for GT track matching (required for Stage 1 agent alignment)
                'gt_bboxes_3d',
                'gt_labels_3d',
                'gt_past_traj',
                'gt_past_traj_mask',
...
data = dict(
    ...
    train_fusionad_with_track_gt=dict(
        type=dataset_type,
        file_client_args=file_client_args,
        data_root=data_root,
        test_mode=True,
